...
func (factory *containerFactory) FindContainersForDeletion() ([]CreatingContainer, []CreatedContainer, []DestroyingContainer, error) {
	query, args, err := psql.Select("c.id, c.handle, c.worker_name, c.hijacked, c.discontinued, c.state").
		...
		LeftJoin("volumes v ON (v.resource_cache_id = c.worker_resource_cache_id AND v.worker_name = c.worker_name)").
		LeftJoin("(select resource_cache_id, count(*) cnt from resource_cache_uses GROUP BY resource_cache_id) rcu ON rcu.resource_cache_id = c.worker_resource_cache_id").
		Where(sq.Or{
