		...
		Context("when there is resource cache", func() {
			...
			Context("when resource cache is not used any more", func() {
				...
				Context("when there is a volume for resource cache in created state", func() {
					BeforeEach(func() {
						creatingContainer, err := defaultTeam.CreateContainer(defaultWorker.Name(), db.NewBuildStepContainerOwner(build.ID(), "some-step"), fullMetadata)
						Expect(err).NotTo(HaveOccurred())

						creatingVolume, err := volumeFactory.CreateContainerVolume(defaultTeam.ID(), defaultWorker, creatingContainer, "some-body-once-told-me-the-world-is")
						Expect(err).NotTo(HaveOccurred())
				...
				Context("when there is a volume for resource cache in destroying state", func() {
					BeforeEach(func() {
						...
						creatingVolume, err := volumeFactory.CreateContainerVolume(defaultTeam.ID(), defaultWorker, creatingContainer)
						Expect(err).NotTo(HaveOccurred())
