...
var _ = Describe("Keeping track of containers", func() {
	...
	FIt("differentiates between a single resource's checking containers with different stages", func() {
		checkStageAContainerID := db.ContainerIdentifier{
			ResourceID:          1,
			CheckSource:         atc.Source{"some": "source"},
			CheckType:           "some-type",
		...
		resourceCache, err := dbngResourceCacheFactory.FindOrCreateResourceCacheForResource(
			logger,
			1,
			"some-type",
		...
		Expect(err).NotTo(HaveOccurred())
		creatingContainer, err = dbngTeam.CreateResourceGetContainer(dbngWorker, resourceCache, "some-step")
		Expect(err).NotTo(HaveOccurred())
		createdContainer, err = creatingContainer.Created()
		Expect(err).NotTo(HaveOccurred())
		_, err = database.UpdateContainerTTLToBeRemoved(db.Container{
