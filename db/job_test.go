...
var _ = Describe("Job", func() {
	...
	Describe("FinishedAndNextBuild", func() {
		...
		It("can report a job's latest running and finished builds", func() {
			...
			started, err := nextBuild.Start("some-engine", `{"id":"1"}`)
			Expect(err).NotTo(HaveOccurred())
			...
			otherStarted, err := otherNextBuild.Start("some-engine", `{"id":"1"}`)
			Expect(err).NotTo(HaveOccurred())
			...
			started, err = anotherRunningBuild.Start("some-engine", `{"meta":"data"}`)
			Expect(err).NotTo(HaveOccurred())
			Expect(started).To(BeTrue())
