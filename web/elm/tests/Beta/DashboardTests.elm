...
pipelineStatus =
    let
        ...
        pipeline paused jobs =
            { pipeline = { paused = paused }, jobs = (RemoteData.Success jobs) }
    in
        describe "many statuses of a pipeline"
            ...
            , test "returns status running for paused and running jobs" <|
                \_ ->
                    Dashboard.pipelineStatus (pipeline False [ runningJob, pausedJob ])
                        |> Expect.equal Concourse.PipelineStatusRunning
